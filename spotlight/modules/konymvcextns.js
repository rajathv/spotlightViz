      ...
      if (this.moduleConfig !== undefined) {
        ...
        if (forms !== undefined) {
          for (var i = 0; i < forms.length; i++) {
            if (kony.mvc.registry.getControllerType(forms[i].FormName) != constants.FORM_CONTROLLER_TYPE) {
              var formName = forms[i].FormName;
              var friendlyName = forms[i].friendlyName || formName;
              var controller = forms[i].Controller;
              var formController = forms[i].FormController;
              var controllerExtensions = forms[i].ControllerExtensions;
              var controllerExtName = [controller];
              Array.prototype.push.apply(controllerExtName, controllerExtensions);
              kony.mvc.registry.remove(friendlyName, formName);
              kony.mvc.registry.add(friendlyName, formName, {
                'controllerName': friendlyName,
