define([], function() {
  	...
  	function TestCase_CustomerCare_addNewCustomerCareInfo(commandId) {
        kony.mvc.Business.CommandHandler.call(this, commandId);
    }
	
    inheritsFrom(TestCase_CustomerCare_addNewCustomerCareInfo, kony.mvc.Business.CommandHandler);
  
  	TestCase_CustomerCare_addNewCustomerCareInfo.prototype.execute = function(command){
		var customerCareModule = kony.mvc.MDAApplication.getSharedInstance().getModuleManager().getModule("MasterDataModule");
        var self = this;
        var expect = chai.expect;
        var inputParam = {
           "Communication_Records":{
             "Description":"1234567890 DESC",
             "Extension":"ext1",
             "Priority":"1",
   ...
   try{
               if(viewModel&&viewModel.contacts){
                    ...
                    customerCareModule.presentationController.addNewCustomerCareInfo(inputParam);
              }
   }catch(e){
    self.sendResponse(command, kony.mvc.constants.STATUS_Failure, e);
   }
	...
	TestCase_CustomerCare_addNewCustomerCareInfo.prototype.validate = function(){
		
    };
    
    return TestCase_CustomerCare_addNewCustomerCareInfo;
