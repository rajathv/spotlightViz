define([], function() {

  	function TestCase_Messages_draftMessage(commandId) {
        kony.mvc.Business.CommandHandler.call(this, commandId);
    }
	
    inheritsFrom(TestCase_Messages_draftMessage, kony.mvc.Business.CommandHandler);
  
  	TestCase_Messages_draftMessage.prototype.execute = function(command){
		var CSRModule = kony.mvc.MDAApplication.getSharedInstance().getModuleManager().getModule("CSRModule");
        var self = this;
        var expect = chai.expect;
      var input = {
          "attachments":[],
          "accountid":"1",
          "customer_id":"",
          "messagedescription":"",
          "messagestatus":"SENT",
          "priority":"High",
          "requestcategory_id":"RCID_ACCOUNTS",
          "requestid":"",
          "requestmessage_id":"",
          "requeststatus":"SID_INPROGRESS",
          "requestsubject":"ashbfhsc",
          "username":",Cavanaugh@123"
        };
      function onCompletion(viewModel) {
            try {
				if(viewModel && viewModel.message){
					expect(viewModel, 'to get Messages').to.include.keys(["message"]);
                self.sendResponse(command, kony.mvc.constants.STATUS_SUCCESS, viewModel);
				}
            } catch (e) {
                self.sendResponse(command, kony.mvc.constants.STATUS_Failure, e);
            }
        }
        function onFetchCompletion(viewModel){
   try{
               if(viewModel){
                    command.context.onPresentForm('frmCSR', onCompletion);
                    CSRModule.presentationController.draftMessage(input);
              }
   }catch(e){
    self.sendResponse(command, kony.mvc.constants.STATUS_Failure, e);
   }
  }
        try {
            command.context.onPresentForm('frmCSR', onFetchCompletion);
            CSRModule.presentationController.navigateTo('CSRModule','showCSR');
        } catch (e) {
            this.sendResponse(command, kony.mvc.constants.STATUS_Failure, e);
        }
    };
	
	TestCase_Messages_draftMessage.prototype.validate = function(){
		
    };
    
    return TestCase_Messages_draftMessage;
    
});