define([], function() {
  	...
  	function TestCase_OutageMessages_EditOutageMessage(commandId) {
        kony.mvc.Business.CommandHandler.call(this, commandId);
    }
	
    inheritsFrom(TestCase_OutageMessages_EditOutageMessage, kony.mvc.Business.CommandHandler);
  
  	TestCase_OutageMessages_EditOutageMessage.prototype.execute = function(command){
		var messagesModule = kony.mvc.MDAApplication.getSharedInstance().getModuleManager().getModule("OutageMessageModule");
        var self = this;
        var expect = chai.expect;
		var editInputParam=[{"Service_id":"","MessageText":"","Status_id":"","id":"","modifiedby":"UID11"}];
		  ...
		  function onFetchCompletion(viewModel){
			try{
               if(viewModel&&viewModel.outageMessageList){
					...
					editInputParam[0].MessageText=viewModel.outageMessageList[0].MessageText+"edited";
                    messagesModule.presentationController.editOutageMessage(editInputParam);
              }
   }catch(e){
    self.sendResponse(command, kony.mvc.constants.STATUS_Failure, e);
   }
	...
	TestCase_OutageMessages_EditOutageMessage.prototype.validate = function(){
		
    };
    
    return TestCase_OutageMessages_EditOutageMessage;
