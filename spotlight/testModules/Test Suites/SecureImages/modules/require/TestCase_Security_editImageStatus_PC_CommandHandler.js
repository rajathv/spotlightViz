define([], function() {
  ...
  function TestCase_Security_editImageStatus_PC_CommandHandler(commandId) {
    kony.mvc.Business.CommandHandler.call(this, commandId);
  }

  inheritsFrom(TestCase_Security_editImageStatus_PC_CommandHandler, kony.mvc.Business.CommandHandler);

  TestCase_Security_editImageStatus_PC_CommandHandler.prototype.execute = function (command) {
    var self = this;
    ...
    function onGetImagesCompletion(getImagesResponse) {
      if(getImagesResponse.data.records.length > 0) {
        try {
          ...
          command.context.onPresentForm('frmSecureImage', onEditImageStatusCompletion);
          securityModule.presentationController.editImage(testInput);

          function onEditImageStatusCompletion(viewModel) {
  ...
  TestCase_Security_editImageStatus_PC_CommandHandler.prototype.validate = function(){
  };

  return TestCase_Security_editImageStatus_PC_CommandHandler;
